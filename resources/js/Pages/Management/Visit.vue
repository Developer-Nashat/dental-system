...
const filterVisitsByPeriod = (period: Period) => {
  ...
  switch (period) {
    ...
    case Period.YESTERDAY:
      ...
      return props.visits.filter(
        (visit: Visit) =>
          new Date(visit.visit_date) >= new Date(yesterday.setHours(0, 0, 0, 0)) &&
          new Date(visit.visit_date) <= new Date(yesterday.setHours(23, 59, 59, 999))
      );
    case Period.TOMORROW:
      const tomorrow = new Date(today.setDate(today.getDate() + 1));
      return props.visits.filter(
        (visit: Visit) =>
          new Date(visit.visit_date) >= new Date(tomorrow.setHours(0, 0, 0, 0)) &&
          new Date(visit.visit_date) <= new Date(tomorrow.setHours(23, 59, 59, 999))
      );
    ...
    case Period.THIS_MONTH:
      const startOfMonth = new Date(today.getFullYear(), today.getMonth(), 1); // أول يوم بالشهر
      const endOfMonth = new Date(today.getFullYear(), today.getMonth() + 1, 0); // آخر يوم بالشهر
      return props.visits.filter(
        (visit: Visit) =>
...
const openPatientVisit = (visit: Visit, row: any) => {
  router.visit(route("PatientVisit", row.item.id), {
    method: "get",
    preserveState: true,
  });
};
...
const getSelectedPeriod = (period: Period) => {
  selectedPeriod.value =
    (Object.entries(Period).find(([k, v]) => v === period)?.[0] as Period) ??
    Period.TODAY;
  ...
  const endDate = new Date(customDateRange?.value[customDateRange?.value.length - 1]);
  ...
  <v-card max-width="90%" class="mx-auto mt-16" elevation="4">
    ...
    >
      ...
      <div class="align-center">
        ...
        >
          ...
          <v-chip class="chip-hover" variant="outlined" @click="customDateDialog = true"
            >مخصص</v-chip
          >
        </v-chip-group>
      </div>
      ...
      <v-dialog v-model="customDateDialog" persistent max-width="400px">
        <v-card>
          ...
          <v-card-actions>
            <VBtnCancel @click="customDateDialog = false">إلغاء</VBtnCancel>
            <VBtnSave @click="getSelectedPeriod(Period.CUSTOME)">موافق</VBtnSave>
          </v-card-actions>
        </v-card>
      </v-dialog>
    </v-card-title>
    ...
    >
        ...
        <v-toolbar flat class="bg-white border-t border-b">
          ...
          <v-dialog v-model="dialog" max-width="60%" persistent>
            <template v-slot:activator="{ props }">
              <VBtnAdd v-bind="props" @click="formInit"> تسجيل زيارة </VBtnAdd>
            </template>
            ...
            <v-card>
              ...
              <v-form @submit.prevent="saveVisit">
                  ...
                  <v-row>
                    ...
                    <v-col cols="12" md="6" sm="12">
                      <v-text-input
                        ...
                        readonly
                      >
                        <v-dialog v-model="timeDialog" activator="parent" width="auto">
                          <v-time-picker
                            title="إختار الوقت"
                            color="primary"
                ...
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn-cancel @click="closeDialog"> إلغاء </v-btn-cancel>
                  <v-btn-save type="submit" :disabled="form.processing"> حفظ </v-btn-save>
                </v-card-actions>
              </v-form>
            </v-card>
          </v-dialog>
          ...
          >
            <v-card>
              <v-form @submit.prevent="deleteItemConfirm" method="delete">
                <v-card-title class="text-h5 pb-8 my-4 text-center" height="8">
                  <v-icon color="red-darken-1" size="100">mdi-help-circle-outline</v-icon>
                </v-card-title>
                <v-card-text class="text-grey-darken-4 text-center mb-4">
                ...
                <v-card-actions class="my-1">
                  <v-spacer></v-spacer>
                  <v-btn-cancel variant="text" @click="closeDeleteDialog"
                    >إلغاء</v-btn-cancel
                  >
                  <v-btn-delete class="mr-12" type="submit" :disabled="form.processing"
                    >نعم
                    <v-icon class="pr-4 pt-1" size="large">mdi-delete</v-icon>
                  </v-btn-delete>
                  <v-spacer></v-spacer>
                </v-card-actions>
              </v-form>
      ...
      <template v-slot:item.actions="{ item }">
        <v-icon color="primary" class="me-4" @click.stop="editVisit(item as Visit)">
          mdi-pencil
        </v-icon>
        <v-icon @click.stop="deleteVisit(item as Visit)" color="red-darken-1" hint="حذف">
          mdi-delete
        </v-icon>
      </template>

      <template v-slot:no-data>
        <div class="font-weight-bold text-grey-darken-1">لايوجد زيارات</div>
      </template>
    </v-data-table>
  </v-card>
</template>
